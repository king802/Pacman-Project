...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # Pacman by checking if it has a noisyDistance of None
        if noisyDistance is None:
            # put it in the jail in this case (1,1)
            allPossible[self.getJailPosition()] = 1.0

        # for all positions in the list of legal positions find the manhattan distance between that poistion and
        # the pacmans position.
        for position in self.legalPositions:
            trueDistance = util.manhattanDistance(position, pacmanPosition)
            # if emissionModel[trueDistance] > 0: its a probability and shouldn't ever be less then zero
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # need to count through beliefs like above
        allPossible = util.Counter()

        # newPostDist[p] = Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )
        # print self.getPositionDistribution(self.setGhostPosition(gameState, (1,9))).items()
        # [((1.0, 10.0), 0.3333333333333333),|| ((1.0, 8.0), 0.3333333333333333),|| ((1.0, 9.0), 0.3333333333333333)]
        # this basically says from position (1,9) there is a .33 percent chance of moving to 10.0 and so on...

        # for all of the positions in the legal positions list
        for position in self.legalPositions:
            # get the distribution of possible newpositions that can occur from the current position
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, position))
            # loop over each individual possible new position with its corresponding probability of happening
            for newPos, prob in newPosDist.items():
                # update the beliefs at that possible position by the current position times the probability it moves to
                # the new position.
                allPossible[newPos] += self.beliefs[position] * prob

        # This is the same as above but you have to normalize and then adjust the beliefs.
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # For this part you want to iterate through the number of particles and assign them a legal position on the
        # board. The key here is that the number of positions can be greater or less then the number of particles so
        ...
        while not len(initialList) == numberOfParticles:  # loops until all particles have been initialized
            for position in legalPositions:  # loops through legal positions and assigns them uniformly
                if not len(
                        initialList) == numberOfParticles:  # need too check to see if we are done for each as position
                    initialList.append(position)  # grow list
                else:
                    break

        self.particles = initialList
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        # The following code first tackles part one of this problem and checks to see if the ghost has been captured.
        # If it has been then it will make all of the particles (guesses of ghost position) say that it is in jail. The
        # second part of this uses what we implemented in the 'self.getBeliefDistribution()' code. It loops through the
        # all of the belief and calculates the distance from them to Pacman. This is very similar to the orginal problem
        # after it has done that though it then updates its belief by [emissions * belief]. Once the beliefs have been
        # updated its time to do part two. first thing we check is to see if the particles have all been assigned to 0.
        # If that is the case then we just call the function we first implemented in this problem. Otherwise we then
        # just re-sample the particles.

        numberOfParticles = self.numParticles
        beliefsCounter = self.getBeliefDistribution()
        ...
        if noisyDistance is None:
            ...
            for position in beliefsCounter:
                trueDistance = util.manhattanDistance(position, pacmanPosition)
                temp = emissionModel[trueDistance] * beliefsCounter[position]
                beliefsCounter[position] = temp

            if beliefsCounter.totalCount() == 0:  # Part 2
                self.initializeUniformly(gameState)
            else:
                newParticles = []
                while not len(newParticles) == numberOfParticles:
                    newParticles.append(util.sample(beliefsCounter))
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # just need to read in the current particles (soon to be old) and then simply use them to get new distribution
        # and then its just a matter of taking a sample and updating.
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # In this section of code all that is happening is that the utilCounter is being created/updated (depends when
        # this is called. But either way it will iterate the current counter by 1 all around. NOTE: if we created an
        # initializing counter somewhere else we could simplify this code by using util.Counter methods to iterate all.
        beliefDistribution = util.Counter()
        listOfParticle = self.particles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # This is going to be almost identical to the previous time in this PA when we had to right a initialization
        # Except this time the legal positions of the ghost are different because there is more then 1. So all I did
        # was substitute out the different permutations of the legal positions for every ghost instead of just the
        # legal positions.

        initialList = []
        numberOfParticles = self.numParticles
        listOfGhostsPossiblePositions = self.legalPositions
        numberOfGhosts = self.numGhosts
        legal = self.legalPositions

        # >>> print list(product([1,2,3],repeat = 2))
        # [(1, 1), (1, 2), (1, 3), (2, 1), (2, 2), (2, 3), (3, 1), (3, 2), (3, 3)]
        actualPositions = list(itertools.product(legal, repeat=numberOfGhosts))

        # w =[1,2,3,4,5]
        # random.shuffle(w)
        # print w -> [4, 3, 2, 1, 5]
        random.shuffle(actualPositions)

        while not len(initialList) == numberOfParticles:
            for position in actualPositions:
                if not len(initialList) == numberOfParticles:
                    initialList.append(position)
                else:
                    break
        self.particles = initialList
    ...
    def observeState(self, gameState):
        ...
        beliefsCounter = util.Counter()
        listOfParticles = self.particles
        numberOfParticles = self.numParticles

        # for all the particles in the list of particles
        for particle in listOfParticles:
            vars = 1
            # loop through the ghosts
            for ghost in range(self.numGhosts):
                # because of multiple ghost this part 1 has to go in line.
                if noisyDistances[ghost] is None:
                    particle = self.getParticleWithGhostInJail(particle, ghost)
                else:
                    emissionModelGhost = emissionModels[ghost]
                    trueDistance = util.manhattanDistance(particle[ghost], pacmanPosition)
                    temp = emissionModelGhost[trueDistance] * vars
                    vars = temp
            beliefsCounter[particle] += vars

        # Same as the above with the case 2!
        self.beliefs = beliefsCounter
        if beliefsCounter.totalCount() == 0:
            self.initializeParticles()  # self.initializeUniformly(gameState)
        else:
            newParticles = []
            while not len(newParticles) == numberOfParticles:
                newParticles.append(util.sample(beliefsCounter))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            prevGhostPositions = newParticle  # this is the list(oldParticles)
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions), i,
                                                             self.ghostAgents[i])
                # newParticles.append(util.sample((newPosDist))) # this is same as previous code
                newParticle[i] = util.sample(newPosDist)  # tuple limited so cant do same code as last time
                # OTHER CODE
                # oldParticles = self.particles
                # updatedParticles = []
                #
            ...
            "*** END YOUR CODE HERE ***"
            listNewParticles.append(tuple(newParticle))
        self.particles = listNewParticles

    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        # This is identical to the previous problem. Just need to make the counter and increment it by 1 each time this
        # function is called.
        beliefDistribution = util.Counter()
        listOfParticle = self.particles
